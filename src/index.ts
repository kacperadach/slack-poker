...
async function drillGto(env, context, payload) {
	await context.say({ text: `<@${context.userId}> is drilling GTO! :drill-gto:` });
}
...
async function bet(env, context, payload) {
	...
	const betAmount = parseFloat(
		messageText.replace('i choose to', '').replace('im gonna go ahead and', '').replace('bet', '').replace('donk', '').trim()
	);
