...
describe("Poker Durable Object", () => {
  ...
  /**
   * Game Scenario 11b: Pre-deal with showdown works correctly
   *
   * Tests:
   * - Pre-deal during active round that goes to showdown
   * - Both showdown (reveal hands, distribute pot) and next round happen
   * - Verifies the lastShowdownSnapshot mechanism works via unit test below
   */
  it("game scenario 11b - pre-deal with showdown works correctly", async () => {
    vi.spyOn(Math, "random").mockReturnValue(0.45);
    const workspaceId = "test-workspace-11b";
    const channelId = "test-channel-11b";
    const sayFn = vi.fn();
    const postEphemeralFn = vi.fn();
    ...
    // === SETUP ===
    await newGame(env, contextUser1, createGenericMessageEvent("alice"));
    await joinGame(env, contextUser1, createGenericMessageEvent("alice"));
    await joinGame(env, contextUser2, createGenericMessageEvent("bob"));
    await buyIn(
      env,
      contextUser1,
      createGenericMessageEvent("alice", "buy in 500")
    );
    await buyIn(
      env,
      contextUser2,
      createGenericMessageEvent("bob", "buy in 500")
    );
    sayFn.mockClear();

    // Start round
    await startRound(env, contextUser1, createGenericMessageEvent("alice"));
    sayFn.mockClear();

    // Queue pre-deal during active round
    await preDeal(env, contextUser1, createGenericMessageEvent("alice"));
    sayFn.mockClear();

    // Play through to showdown by calling/checking all streets
    await call(env, contextUser2, createGenericMessageEvent("bob")); // SB calls
    sayFn.mockClear();
    await check(env, contextUser1, createGenericMessageEvent("alice")); // BB checks -> Flop
    sayFn.mockClear();

    await check(env, contextUser2, createGenericMessageEvent("bob"));
    sayFn.mockClear();
    await check(env, contextUser1, createGenericMessageEvent("alice")); // -> Turn
    sayFn.mockClear();

    await check(env, contextUser2, createGenericMessageEvent("bob"));
    sayFn.mockClear();
    await check(env, contextUser1, createGenericMessageEvent("alice")); // -> River
    sayFn.mockClear();

    await check(env, contextUser2, createGenericMessageEvent("bob"));
    sayFn.mockClear();

    // Final check triggers showdown AND pre-deal starts next round
    await check(env, contextUser1, createGenericMessageEvent("alice"));

    // Verify game transitioned to new round (pre-deal triggered)
    const gameStateFinal = await getGameState(stub, workspaceId, channelId);
    expect(gameStateFinal.gameState).toBe(GameState.PreFlop);

    // Verify showdown happened (had and pot messages)
    const allMessages = sayFn.mock.calls.map((c) => c[0].text).join("\n");
    expect(allMessages).toContain(" had ");
    expect(allMessages).toContain("Main pot");

    // The current game state should have empty community cards (new round)
    expect(gameStateFinal.communityCards).toHaveLength(0);
    
    // Verify the next round started
    expect(allMessages).toContain("Starting round with");
  });

  /**
   * Unit test: Verify lastShowdownSnapshot is captured in endRound when pre-deal is active
   */
  it("captures lastShowdownSnapshot when pre-deal triggers new round", () => {
    ...
    players[0].addChips(500);
    players[1].addChips(500);
    
    // Start round
    game.startRound("player1");
    
    // Queue pre-deal
    game.preDeal("player1");
    
    // Get state before fold - should not have snapshot yet
    const stateBefore = game.getState();
    expect(stateBefore.lastShowdownSnapshot).toBeUndefined();
    
    // Player 2 folds, ending the round and triggering pre-deal
    game.fold("player2");
    
    // Get state after fold - should have snapshot with previous round's data
    const stateAfter = game.getState();
    expect(stateAfter.lastShowdownSnapshot).toBeDefined();
    expect(stateAfter.lastShowdownSnapshot?.communityCards.length).toBeGreaterThanOrEqual(0);
    expect(stateAfter.lastShowdownSnapshot?.activePlayers.length).toBe(2);
    
    // The current game state should be reset for new round
    expect(stateAfter.communityCards).toHaveLength(0);
    expect(stateAfter.gameState).toBe(GameState.PreFlop);
  });
