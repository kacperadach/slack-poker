...
test('Blinds are taken correctly and 3rd player must call big blind', () => {
	...
	assert.equal(game.check(currentPlayer.getId()), 'Cannot check, there are active bets');
...
test('Game progresses to flop with raise and fold', () => {
	...
	assert.equal(game.bet(raisingPlayer.getId(), raiseAmount), 'Bet ' + raiseAmount);
	assert.equal(raisingPlayer.getChips(), 1000 - game.getBigBlind() - raiseAmount);
	...
	const winningPlayer = raisingPlayer.getChips() > foldingPlayer.getChips() ? raisingPlayer : foldingPlayer;
	assert.equal(winningPlayer.getChips(), 1000 + game.getBigBlind() * 2 + raiseAmount - game.getBigBlind() - raiseAmount);
});
...
test('Test players leaving and joining and dealer position', () => {
	...
	while (game.getGameState() !== GameState.WaitingForPlayers) {
		const currentPlayer = game.getCurrentPlayer();
		assert(currentPlayer);
		if (game.getCurrentBetAmount() > 0 && currentPlayer.getCurrentBet() < game.getCurrentBetAmount()) {
			assert.equal(game.call(currentPlayer.getId()), Success);
		} else {
...
test('Test player cant afford blinds goes all-in', () => {
	...
	assert.equal(game.getCurrentPlayer()?.getId(), PLAYER_2);
	assert.equal(game.call(PLAYER_2), 'No need to call - already matched the current bet');
	assert.equal(game.check(PLAYER_2), Success);
...
// TODO: add/test blind avoidance algorithm
// TODO: add event listener/emitter
...
- Start Game
    - Active Players in Round
- Show cards dealt
- Players who paid blinds
- Who's turn it is
- Actions:
    - Check
    - Call (amount)
    ...
    - Fold
- Player goes all-in
- Game Progression (Flop, Turn, River, Showdown)
    - Community Cards
- All Players Hand (type i.e. flush, straight etc)
- Winner(s) + winnings/side pots
- Game over
