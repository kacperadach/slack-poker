...
export class TexasHoldem {
	...
	private shouldSkipToShowdown(): boolean {
		...
		const activeNonFoldedPlayers = this.activePlayers.filter((player) => !this.foldedPlayers.has(player.getId()) && !player.getIsAllIn());
	...
	private isBettingRoundComplete(): boolean {
		...
		const activePlayers = this.activePlayers.filter((player) => !this.foldedPlayers.has(player.getId()));
		...
		const allPlayersMatched = activePlayers.every((player) => {
			return player.getCurrentBet() === this.currentBetAmount || player.getIsAllIn();
		});
		...
		if (allPlayersMatched) {
			const notAllInPlayers = activePlayers.filter((player) => !player.getIsAllIn());
			if (notAllInPlayers.length <= 1) return true;
			return activePlayers.every((player) => player.getHadTurnThisRound() || player.getIsAllIn());
		}
	...
	public startRound(playerId: string): string {
		if (this.gameState !== GameState.WaitingForPlayers) {
			this.events.push(new GameEvent(`${playerId} Cannot start round, game is already active!`));
			return 'Game is already active';
		}

		const player = this.activePlayers.find((p) => p.getId() === playerId);
		if (!player) {
			this.events.push(new GameEvent(`${playerId} Player not found or not active!`));
		...
		this.activePlayers.forEach((player) => {
			if (player.getChips() <= 0) {
				this.events.push(new GameEvent(`${player.getId()} is being removed from the game for having no chips`));
	...
	public getCurrentPlayer(): Player | null {
		if (this.gameState === GameState.WaitingForPlayers || this.activePlayers.length === 0) {
			return null;
		}
	...
	public addPlayer(id: string): boolean {
		...
		if (this.activePlayers.length >= 10) {
			this.events.push(new GameEvent(`${id} can't join there are too many players!`));
			return false;
		}
		...
		const inactivePlayerIndex = this.inactivePlayers.findIndex((p) => p.getId() === id);
		...
		if (this.gameState === GameState.WaitingForPlayers) {
			...
			this.events.push(new GameEvent(`${id} Will join the game once this round is over!`));
		}
	...
	public removePlayer(playerId: string): boolean {
		...
		// 	return false;
		// }
		const playerIndex = this.activePlayers.findIndex((player) => player.getId() === playerId);
		if (playerIndex === -1) {
		...
		if (this.gameState !== GameState.WaitingForPlayers) {
			...
			this.events.push(new GameEvent(`${playerId} will leave once the round is over!`));
			return true;
		} else {
	...
	public buyIn(playerId: string, chips: number): string {
		...
		if (this.gameState !== GameState.WaitingForPlayers) {
			this.events.push(new GameEvent(`${playerId} Cannot buy in during active round!`));
		...
		this.events.push(new GameEvent(`${playerId} Bought-in for ${roundedChips} chips`));
		return Success;
	}

	public cashOut(playerId: string): boolean {
		if (this.gameState !== GameState.WaitingForPlayers) {
			this.events.push(new GameEvent(`${playerId} Cannot cash out during active round!`));
			return false;
		}

		const player = this.activePlayers.find((p) => p.getId() === playerId) || this.inactivePlayers.find((p) => p.getId() === playerId);
	...
	public dealInitialCards(): void {
		...
		this.activePlayers.forEach((player) => {
			this.events.push(new GameEvent(`${player.getId()} your cards:`, player.getCards(), true, player.getId()));
		});
	}
	...
	public fold(playerId: string): string {
		...
		if (this.gameState === GameState.WaitingForPlayers) {
			this.events.push(new GameEvent(`${playerId} Cannot fold, round has not started!`));
			return 'Cannot fold, game is not active';
		}

		const player = this.getCurrentPlayer();
		if (!player || player.getId() !== playerId) {
			this.events.push(new GameEvent(`${playerId} Cannot fold, not your turn!`));
	...
	public check(playerId: string): string {
		if (this.gameState === GameState.WaitingForPlayers) {
			this.events.push(new GameEvent(`${playerId} Cannot check, round has not started!`));
			return 'Cannot check, game is not active';
		}

		const player = this.getCurrentPlayer();
		if (!player || player.getId() !== playerId) {
			this.events.push(new GameEvent(`${playerId} Cannot check, not your turn!`));
	...
	public bet(playerId: string, amount: number): string {
		if (this.gameState === GameState.WaitingForPlayers) {
			this.events.push(new GameEvent(`${playerId} Cannot bet, game is not active!`));
		...
		// Validate bet amount
		if (roundedAmount <= 0) {
			this.events.push(new GameEvent(`${playerId} Cannot bet, amount must be positive!`));
		...
		if (betAmount > maxWinnable) {
			this.events.push(new GameEvent(`${playerId} Cannot bet more than ${maxWinnable} - no reason to bet more than you can win!`));
	...
	private handleSidePots(): void {
		...
		const activeNonFoldedPlayers = this.activePlayers.filter((player) => !this.foldedPlayers.has(player.getId()));

		// If only one player remains, they get the entire pot
		if (activeNonFoldedPlayers.length === 1) {
			const winner = activeNonFoldedPlayers[0];
			this.events.push(new GameEvent(`${winner.getId()} wins ${this.currentPot} chips!`));
			winner.addChips(this.currentPot);
			return;
		}

		// Get all players who went all-in, sorted by their total contribution
		const allInPlayers = activeNonFoldedPlayers.filter((player) => player.getIsAllIn()).sort((a, b) => a.getTotalBet() - b.getTotalBet());

		// If all all-in players have the same bet amount, no side pots needed
		if (
			allInPlayers.length === 0 ||
			(activeNonFoldedPlayers.length > 0 && activeNonFoldedPlayers.every((p) => p.getTotalBet() <= allInPlayers[0].getTotalBet()))
		) {
		...
		allInPlayers.forEach((player) => {
			...
			this.activePlayers.forEach((p) => {
				...
				} else if (this.foldedPlayers.has(p.getId()) && p.getTotalBet() > previousBet) {
			...
			const eligiblePlayers = activeNonFoldedPlayers.filter((p) => p.getTotalBet() >= currentBet);
		...
		if (remainingPot > 0) {
			const eligiblePlayers = activeNonFoldedPlayers.filter((p) => !p.getIsAllIn());
			this.distributePot(eligiblePlayers, remainingPot, false);
		}
	}

	private distributePot(eligiblePlayers: Player[], potSize: number, isSidePot: boolean): void {
		...
		const winners = playerHands.filter((ph) => bestHandPlayerIds.includes(ph.player.getId()));
		// Split the pot among winners
		const winnings = potSize / winners.length;
		winners.forEach((winner) => winner.player.addChips(winnings));

		this.events.push(new GameEvent('Community Cards:', this.getCommunityCards()));

		playerHands.forEach(({ player, hand }) => {
			this.events.push(new GameEvent(`${player.getId()} had ${hand.description}`, player.getCards()));
		});

		if (isSidePot) {
			this.events.push(new GameEvent(`Side pot of ${potSize} won by: ${winners.map((w) => w.player.getId()).join(', ')}`));
		} else {
			this.events.push(new GameEvent(`Main pot of ${potSize} won by: ${winners.map((w) => w.player.getId()).join(', ')}`));
		}
	}

	public evaluateHand(cards: Card[]): any {
		if (cards.length !== 7) {
			throw new Error('A hand must consist of exactly 7 cards (2 hole cards + 5 community cards)');
		}
	...
	public getBestHand(hands: any[]): string[] {
		...
		const winningPlayerIds = hands.filter((h) => h.hand.value === minHandValue).map((h) => h.player.getId());
	...
	public showCards(playerId: string, reveal: boolean = false, showCommunityCards: boolean = true): void {
		...
		if (showCommunityCards && this.communityCards.length > 0) {
			this.events.push(new GameEvent('Community Cards:', this.communityCards, !reveal, playerId));
		}
	...
	public getGameStateEvent(): void {
		...
		const allPlayers = [...this.getActivePlayers(), ...this.getInactivePlayers()];
	...
	public static fromJson(data: any): TexasHoldem {
		...
		const game = new TexasHoldem(
			...
			data.activePlayers.map((playerJson: string) => Player.fromJson(playerJson)),
			data.inactivePlayers.map((playerJson: string) => Player.fromJson(playerJson)),
			data.currentPot,
			data.dealerPosition,
			data.smallBlind,
			data.bigBlind,
